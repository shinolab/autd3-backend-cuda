...
__device__ cuFloatComplex conj(const cuFloatComplex a) { return make_cuFloatComplex(a.x, -a.y); }
...
__device__ cuFloatComplex mulcr(const cuFloatComplex a, const float b) { return make_cuFloatComplex(a.x * b, a.y * b); }
...
__global__ void get_diagonal_kernel(const cuFloatComplex *a, uint32_t row, uint32_t col, cuFloatComplex *b) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void get_diagonal_kernel(const float *a, uint32_t row, uint32_t col, float *b) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void set_diagonal_kernel_c(const cuFloatComplex *a, uint32_t n, cuFloatComplex *b) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  if (xi >= n) return;
...
__global__ void reciprocal_kernel(const cuFloatComplex *a, const uint32_t row, const uint32_t col, cuFloatComplex *b) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void make_complex2_kernel(const float *re, const float *im, const uint32_t row, const uint32_t col, cuFloatComplex *dst) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void calc_singular_inv_kernel(float *d_s, uint32_t row, uint32_t col, float alpha, cuFloatComplex *p_singular_inv) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;

  if (xi == yi)
    p_singular_inv[yi + xi * row] = make_cuFloatComplex(d_s[xi] / (d_s[xi] * d_s[xi] + alpha), 0.0);
  else
    p_singular_inv[yi + xi * row] = make_cuFloatComplex(0.0, 0.0);
}
...
__global__ void exp_kernel(const cuFloatComplex *a, const uint32_t row, const uint32_t col, cuFloatComplex *b) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void real_kernel(const cuFloatComplex *src, const uint32_t row, const uint32_t col, float *dst) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void imag_kernel(const cuFloatComplex *src, const uint32_t row, const uint32_t col, float *dst) {
  unsigned int xi = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yi = blockIdx.y * blockDim.y + threadIdx.y;
  if (xi >= col || yi >= row) return;
...
__global__ void col_sum_kernel(const float *din, uint32_t m, uint32_t n, float *dout) {
...
void cu_get_diagonal(const float *a, const uint32_t row, const uint32_t col, float *b) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  get_diagonal_kernel<<<grid, block>>>(a, row, col, b);
}

void cu_get_diagonal_c(const cuFloatComplex *a, const uint32_t row, const uint32_t col, cuFloatComplex *b) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  get_diagonal_kernel<<<grid, block>>>(a, row, col, b);
}
...
void cu_set_diagonal_c(const cuFloatComplex *a, const uint32_t n, cuFloatComplex *b) {
  dim3 block(BLOCK_SIZE * BLOCK_SIZE, 1, 1);
  dim3 grid((n - 1) / (BLOCK_SIZE * BLOCK_SIZE) + 1, 1, 1);
  set_diagonal_kernel_c<<<grid, block>>>(a, n, b);
}

void cu_reciprocal(const cuFloatComplex *a, const uint32_t row, const uint32_t col, cuFloatComplex *b) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  reciprocal_kernel<<<grid, block>>>(a, row, col, b);
}

void cu_hadamard_product(const cuFloatComplex *a, const cuFloatComplex *b, const uint32_t row, const uint32_t col, cuFloatComplex *c) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
...
void cu_make_complex2(const float *re, const float *im, const uint32_t row, const uint32_t col, cuFloatComplex *dst) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
...
void cu_calc_singular_inv(float *d_s, const uint32_t row, const uint32_t col, const float alpha, cuFloatComplex *p_singular_inv) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  calc_singular_inv_kernel<<<grid, block>>>(d_s, row, col, alpha, p_singular_inv);
}

void cu_exp(const cuFloatComplex *a, const uint32_t row, const uint32_t col, cuFloatComplex *b) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  exp_kernel<<<grid, block>>>(a, row, col, b);
}

void cu_real(const cuFloatComplex *src, const uint32_t row, const uint32_t col, float *dst) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  real_kernel<<<grid, block>>>(src, row, col, dst);
}

void cu_imag(const cuFloatComplex *src, const uint32_t row, const uint32_t col, float *dst) {
  dim3 block(BLOCK_SIZE, BLOCK_SIZE, 1);
  dim3 grid((col - 1) / BLOCK_SIZE + 1, (row - 1) / BLOCK_SIZE + 1, 1);
  imag_kernel<<<grid, block>>>(src, row, col, dst);
}

void cu_reduce_col(const float *mat, const uint32_t m, const uint32_t n, float *result) {
